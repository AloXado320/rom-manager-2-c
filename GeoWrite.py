...
Cmds={
	0:(lambda x,y:['GEO_BRANCH_AND_LINK(%s)'%('Geo_'+y+hex(B2I(x[4:8]))),8,'PUSH',x[4:8],'ext']),
	1:(lambda x,y:['GEO_END()',4]),
	2:(lambda x,y:['GEO_BRANCH(%d,%s)'%(x[1],'Geo_'+y+hex(B2I(x[4:8]))),8,'PUSH',x[4:8],'ext']),
	3:(lambda x,y:['GEO_RETURN()',4,'POP',0,'ext']),
	4:(lambda x,y:['GEO_OPEN_NODE()',4]),
	5:(lambda x,y:['GEO_CLOSE_NODE()',4]),
	6:(lambda x,y:['GEO_ASSIGN_AS_VIEW(%d)'%((B2I(x[2:4]))),4]),
	7:(lambda x,y:['GEO_UPDATE_NODE_FLAGS(%d,%d)'%(x[1],(B2I(x[2:4]))),4]),
	8:(lambda x,y:['GEO_NODE_SCREEN_AREA(%d,%d,%d,%d,%d)'%(x[3],(B2I(x[4:6])),(B2I(x[6:8])),(B2I(x[8:10])),(B2I(x[10:12]))),12]),
	9:(lambda x,y:['GEO_NODE_ORTHO(%d)'%((B2I(x[2:4]))),4]),
	10:(lambda x,y:['GEO_CAMERA_FRUSTUM{}({},{},{}{}{})'
	.format("_WITH_FUNC" if x[1] else "",
	(B2I(x[2:4])),(B2I(x[4:6])),(B2I(x[6:8])),
	"," if x[1] else "",
	(B2I(x[8:12])) if x[1] else ""),
	(8+min(x[1]*4,4)),'CVASM',x[8:12],'ext']),
	11:(lambda x,y:['GEO_NODE_START()',4]),
	12:(lambda x,y:['GEO_ZBUFFER(%d)'%(x[1]),4]),
	13:(lambda x,y:['GEO_RENDER_RANGE(%d,%d)'%((B2I(x[4:6])),(B2I(x[6:8]))),8]),
	14:(lambda x,y:['GEO_SWITCH_CASE(%d,%d)'%(x[3],(B2I(x[4:8]))),8,'CVASM',x[4:8],'ext']),
	...
	8+min(4,(x[1]>>4==8)*4)]),
	18:(lambda x,y:['GEO_ROTATION_NODE{}({},{},{},{}{}{})'.format("_WITH_DL" if x[1]>>4==8 else "",
	x[1]&0xF,(B2I(x[2:4])),(B2I(x[4:6])),(B2I(x[6:8])),
	"," if x[1]>>4==8 else "",'DL_'+y+hex(B2I(x[8:12])) if x[1]>>4==8 else ""),
	8+min(4,(x[1]>>4==8)*4)]),
	19:(lambda x,y:['GEO_ANIMATED_PART(%d,%d,%d,%d,%s)'%(x[1],(B2I(x[2:4])),(B2I(x[4:6])),(B2I(x[6:8])),'DL_'+y+hex(B2I(x[8:12])) if B2I(x[8:12])>0 else '0'),12,'STOREDL',x[8:12],'ext']),
	20:(lambda x,y:['GEO_BILLBOARD_WITH_PARAMS{}({},{},{},{}{}{})'.format("_AND_DL" if x[1]>>4==8 else "",
	x[1]&0xF,(B2I(x[2:4])),(B2I(x[4:6])),(B2I(x[6:8])),
	"," if x[1]>>4==8 else "",'DL_'+y+hex(B2I(x[8:12])) if x[1]>>4==8 else ""),
	8+min(4,(x[1]>>4==8)*4)]),
	21:(lambda x,y:['GEO_DISPLAY_LIST(%d,%s)'%(x[1],'DL_'+y+hex(B2I(x[4:8])) if B2I(x[4:8])>0 else '0'),8,'STOREDL',x[4:8],'ext']),
	22:(lambda x,y:['GEO_SHADOW(%d,%d,%d)'%(x[3],x[5],
	(B2I(x[6:8]))),8]),
	23:(lambda x,y:['GEO_RENDER_OBJ()',4]),
	24:(lambda x,y:['GEO_ASM(%d,%d)'%((B2I(x[2:4])),(B2I(x[4:8]))),8,'CVASM',x[4:8],'ext']),
	25:(lambda x,y:['GEO_BACKGROUND{}({}{}{})'.format(
	"_COLOR" if not (B2I(x[4:8])) else "",
	(B2I(x[2:4])),
	"," if (B2I(x[4:8])) else "",
	(B2I(x[4:8])) if (B2I(x[4:8])) else ""),8,'CVASM',x[4:8],'ext']),
	26:(lambda x,y:['GEO_NOP_1A()',8]),
	0x1D:(lambda x,y:['GEO_SCALE{}({},{}{}{})'.format("_WITH_DL" if x[1]>>4==8 else "",
	x[1]&0xF,(B2I(x[4:8])),
	"," if x[1]>>4==8 else "",(B2I(x[8:12])) if x[1]>>4==8 else ""),
	8+min(4,(x[1]>>4==8)*4)]),
	28:(lambda x,y:['GEO_HELD_OBJECT(%d,%d,%d,%d,%d)'
	%(x[1],(B2I(x[2:4])),(B2I(x[4:6])),(B2I(x[6:8])),(B2I(x[8:12]))),12]),
	30:(lambda x,y:['GEO_NOP_1E()',8]),
	31:(lambda x,y:['GEO_NOP_1F()',16]),
	32:(lambda x,y:['GEO_CULLING_RADIUS(%d)'%(B2I(x[2:4])),4])
}
